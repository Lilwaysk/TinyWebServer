...
private:
    ...
    static const int MAX_LINES = 50000;                   // 日志文件内容的最长日志条数
...
#define LOG_BASE(level, format, ...) \
    do {\
        ...
        if(log->IsOpen() && log->GetLevel() <= level) {\
            log->write(level, formar, ##__VA_ARGS__); \
            log->flush();\
        }\
    } while(0);
...
/*
    ...
    ...表示可变参数，__VA__ARGS__就是将...的值赋值到这里
    前面加上##的作用是：当可变参数的个数为0时，这里的##可以把前面多余的"."去掉，否则会编译出错。
*/

#define LOG_DEBUG(format, ...) do {LOG_BASE(0,format,##__VA__ARGS__)} while(0);
#define LOG_INFO(format, ...) do {LOG_BASE(1,format,##__VA__ARGS__)} while(0);
#define LOG_WARN(format, ...) do {LOG_BASE(2,format,##__VA__ARGS__)} while(0);
#define LOG_ERROR(format, ...) do {LOG_BASE(3,format,##__VA__ARGS__)} while(0);
